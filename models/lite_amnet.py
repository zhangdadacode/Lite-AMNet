...
import torch
from models.densestack import DenseStack_Backnone
from models.modules import Reg2DDecode3D
from models.loss import l1_loss, normal_loss, edge_length_loss, contrastive_loss_3d, contrastive_loss_2d
...
class Lite_AMNet(nn.Module):
    def __init__(self, cfg):
        ...
        Args:
            cfg : config file
        """
        super(Lite_AMNet, self).__init__()
        self.cfg = cfg
        ...
        # 3D decoding stage: Uses a defined template to guide mesh construction.
    ...
    def forward(self, x):
        if x.size(1) == 6:  
		pred3d_list = []  # Predicted 3D coordinates
		pred2d_pt_list = []  # Predicted 2D coordinates
            for i in range(2):
                latent, pred2d_pt, pre_out, stack1_out, stack2_out = self.backbone(x[:, 3 * i:3 * i + 3]) 


                pred3d = self.decoder3d(pred2d_pt, latent, pre_out, stack1_out, stack2_out)  
...
if __name__ == '__main__':
    ...
    model = Lite_AMNet(cfg)
    model_out = model(torch.randn(2,3, 128, 128))
