...
def main(args):
    ...
    if args.rank == 0:
        print(cfg)
        print(args.exp_name)
    exec('from models.{} import {}'.format(cfg.MODEL.NAME.lower(), cfg.MODEL.NAME))
    exec('from datasets.{} import {}'.format(cfg.TRAIN.DATASET.lower(), cfg.TRAIN.DATASET))
    exec('from datasets.{} import {}'.format(cfg.VAL.DATASET.lower(), cfg.VAL.DATASET))
    ...
    if cfg.MODEL.RESUME:
        ...
        epoch = 0  #Disregard epoch in checkpoint, force training from epoch 0.
    ...
    def safe_collate(batch):
        # Stack tensors explicitly to avoid non-resizable storage from default_collate.
        elem = batch[0]
        if torch.is_tensor(elem):
            return torch.stack([b.contiguous() for b in batch], 0)
        if isinstance(elem, dict):
            return {k: safe_collate([d[k] for d in batch]) for k in elem}
        if isinstance(elem, (list, tuple)):
            return type(elem)(safe_collate(list(items)) for items in zip(*batch))
        return batch

    num_workers = max(2, min(12, (os.cpu_count() or 8) - 2))
    kwargs = {
        "pin_memory": True,
...
if __name__ == "__main__":
    ...
    # args.exp_name = 'test'
    args.config_file = '/home/hdh/桌面/zjj_Datas/Lite-AMNet/configs/Lite-AMNet.yml'
    main(args)
